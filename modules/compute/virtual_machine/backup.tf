resource "azurerm_backup_protected_vm" "backup" {
  ...
  resource_group_name = try(var.settings.backup.vault_key[var.settings.backup.vault_key].resource_group_name, null)
  recovery_vault_name = try(var.settings.backup.vault_key[var.settings.backup.vault_key].name, null)
  backup_policy_id    = try(var.settings.backup.policy_key[var.settings.backup.policy_key].id, null)
  source_vm_id        = try(var.settings.backup.vm_key[var.settings.backup.vm_key].id, null)
